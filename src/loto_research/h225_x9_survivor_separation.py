"""H225-X9: unrestricted exact separators for actual H225-X8 survivors."""
...
X8=ROOT/'data'/'derived'/'h225_x8_incremental_exact_rescreen.json'
OLD=[
...
OUTDIR=ROOT/'data'/'derived'/'h225_x9_separator_shards'
MERGED=ROOT/'data'/'derived'/'h225_x9_survivor_separation.json'
NEW=ROOT/'data'/'derived'/'h225_x9_new_witnesses.json'
JOBS=44

def load_x8():
 d=json.loads(X8.read_text()); assert d['packet']=='H225-X8' and d['chunk_shards']==44 and d['quotient_coefficient_states_screened']==306450 and len(d['sectors'])==11
 assert sum(int(s['quotient_coefficient_states']) for s in d['sectors'])==306450
 return d

def solve_job(job,time_limit):
 assert 0<=job<JOBS
 si,ci=divmod(job,4); d=load_x8(); sec=d['sectors'][si]; ch=sec['chunks'][ci]
 ...
 if ns==0:
  assert nt==0
  return {'packet':'H225-X9','job':job,'sector_index':si,'chunk_index':ci,'skipped_no_survivor':True,'x8_chunk_survivor_states':0,'x8_chunk_survivor_shift_tuples':0,'counterexample_found':False,'conclusive_for_selected_design':True,'n3':None,'witness':None}
 ...
 out={'packet':'H225-X9','job':job,'sector_index':si,'chunk_index':ci,'skipped_no_survivor':False,'x8_chunk_survivor_states':ns,'x8_chunk_survivor_shift_tuples':nt,'representative':[beta,gamma],'coefficients':list(coeff),'coefficient_pairs':[list(map(int,COEFFS[p])) for p in coeff],'shifts':list(shifts),'triples':int(len(triples)),'separator_status':int(res.status),'separator_message':str(res.message),'counterexample_found':separated is not None,'conclusive_for_selected_design':separated is not None}
...
def merge(paths):
 ...
 old=old_keys(); new={k:w for k,w in unique.items() if k not in old}; x8=load_x8()
 merged={'packet':'H225-X9','source_packet':'H225-X8','x8_survivor_states':int(x8['exact_shift_surviving_coefficient_states']),'x8_survivor_shift_tuples':int(x8['exact_surviving_shift_tuples']),'total_chunks':JOBS,'active_survivor_chunks':len(active),'skipped_zero_survivor_chunks':JOBS-len(active),'counterexamples_found':len(found),'inconclusive_active_jobs':len(active)-len(found),'unique_balanced_counterexamples':len(unique),'genuinely_new_witnesses':len(new),'all_selected_active_designs_broken':len(found)==len(active),'jobs':rows}
 wp={'packet':'H225-X9','witness_count':len(new),'witnesses':list(new.values()),'deduplicated_against':['H234','H225-X1','H225-X3','H225-X5','H225-X7'],'validity':'Every stored witness is balanced and returned by unrestricted exact n3<=2 MILP separation.'}
 return merged,wp
