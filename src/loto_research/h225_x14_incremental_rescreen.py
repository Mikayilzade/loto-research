"""H225-X14: full 44-way exact rescreen after H225-X13 separator cuts."""
...
OUTDIR=ROOT/'data'/'derived'/'h225_x14_chunk_shards'
MERGED=ROOT/'data'/'derived'/'h225_x14_incremental_exact_rescreen.json'
base.PACKETS=base.PACKETS+[(X11,'H225-X11'),(X13,'H225-X13')]

def solve_chunk(sector_index,chunk):
 out=base.solve_chunk(sector_index,chunk)
 out['packet']='H225-X14'; out['source_packet']='H225-X13'
 out['x13_raw_new_witnesses']=out.pop('x9_raw_new_witnesses')
 out['x13_affine_expanded_instances']=out.pop('x9_affine_expanded_instances')
 return out

def merge(paths):
 rows=sorted([json.loads(p.read_text()) for p in paths],key=lambda x:(x['sector_index'],x['chunk_index']))
 assert len(rows)==44 and all(x['packet']=='H225-X14' for x in rows)
 ...
 return {'packet':'H225-X14','method':'full_exact_rescreen_after_X13_new_witness_affine_orbits','source_packet':'H225-X13','chunk_shards':44,
  ...
  'x13_raw_new_witnesses':int(rows[0]['x13_raw_new_witnesses']),'x13_affine_expanded_instances':int(rows[0]['x13_affine_expanded_instances']),
  'all_general_cyclic_affine_designs_rejected_by_augmented_witnesses':bool(exstates==0),'sectors':sectors,
  'interpretation':'Zero exact survivors closes H225. Positive survivors require H225-X15 separators.'}
