"""H225-X4: full exact rescreen after H225-X3 separator cuts.

This stage deliberately does not depend on the fragile legacy H225-X2 merged
JSON. It reconstructs the exact witness universe from immutable components:
base H226 witnesses + H234 cuts + H225-X1 cuts + genuinely new H225-X3 cuts,
affine-expands every added witness through the same 128 symmetries, and screens
the unchanged H228 quotient universe of 306,450 coefficient states.
"""
...
def witness_universe():
    ...
    h234=json.loads(H234.read_text()); x1=json.loads(X1.read_text()); x3=json.loads(X3.read_text())
    assert h234['packet']=='H234' and h234['witness_count']==44 and len(h234['witnesses'])==44
    assert x1['packet']=='H240' and len(x1['witnesses'])==x1['witness_count']
    assert x3['packet']=='H225-X3' and len(x3['witnesses'])==x3['witness_count']
    assert x3['witness_count']>0, 'X4 is useful only when X3 produced genuinely new cuts'
    a234=affine_expand(h234['witnesses']); a1=affine_expand(x1['witnesses']); a3=affine_expand(x3['witnesses'])
    W=np.concatenate([base,np.asarray(a234+a1+a3,dtype=np.int16)],axis=0)
    return W,pre_n,h186_n,len(a234),len(a1),len(a3),x1['witness_count'],x3['witness_count']

def solve_chunk(sector_index,chunk):
    W,pre_n,h186_n,h234_exp,x1_exp,x3_exp,x1_raw,x3_raw=witness_universe()
    ...
    return {'packet':'H225-X4','source_packet':'H225-X3','sector_index':sector_index,'chunk_index':chunk,'representative':[int(beta),int(gamma)],
        ...
        'base_expanded_witness_instances':int(len(W)-h234_exp-x1_exp-x3_exp),'h234_raw_witnesses':44,'h234_affine_expanded_instances':h234_exp,
        'x1_raw_witnesses':x1_raw,'x1_affine_expanded_instances':x1_exp,'x3_raw_new_witnesses':x3_raw,'x3_affine_expanded_instances':x3_exp,
...
def merge(paths):
    ...
    return {'packet':'H225-X4','method':'full_exact_rescreen_after_X3_new_witness_affine_orbits','source_packet':'H225-X3','chunk_shards':44,
        ...
        'x3_raw_new_witnesses':rows[0]['x3_raw_new_witnesses'],'x3_affine_expanded_instances':rows[0]['x3_affine_expanded_instances'],
        'all_general_cyclic_affine_designs_rejected_by_augmented_witnesses':bool(exstates==0),'sectors':sectors,
        'interpretation':'Zero exact survivors closes H225. Positive survivors require the next separator packet.'}
