"""H225-X18: full 44-way exact rescreen after H225-X17 separator cuts."""
...
OUTDIR=ROOT/'data'/'derived'/'h225_x18_chunk_shards'
MERGED=ROOT/'data'/'derived'/'h225_x18_incremental_exact_rescreen.json'
base.PACKETS=base.PACKETS+[(X11,'H225-X11'),(X13,'H225-X13'),(X15,'H225-X15'),(X17,'H225-X17')]

def solve_chunk(sector_index,chunk):
 out=base.solve_chunk(sector_index,chunk)
 out['packet']='H225-X18'; out['source_packet']='H225-X17'
 out['x17_raw_new_witnesses']=out.pop('x9_raw_new_witnesses')
 out['x17_affine_expanded_instances']=out.pop('x9_affine_expanded_instances')
 return out

def merge(paths):
 rows=sorted([json.loads(p.read_text()) for p in paths],key=lambda x:(x['sector_index'],x['chunk_index']))
 assert len(rows)==44 and all(x['packet']=='H225-X18' for x in rows)
 ...
 return {'packet':'H225-X18','method':'full_exact_rescreen_after_X17_new_witness_affine_orbits','source_packet':'H225-X17','chunk_shards':44,
  ...
  'x17_raw_new_witnesses':int(rows[0]['x17_raw_new_witnesses']),'x17_affine_expanded_instances':int(rows[0]['x17_affine_expanded_instances']),
  'all_general_cyclic_affine_designs_rejected_by_augmented_witnesses':bool(exstates==0),'sectors':sectors,
  'interpretation':'Zero exact survivors closes H225. Positive survivors require H225-X19 separators.'}
