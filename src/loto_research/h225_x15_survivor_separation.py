"""H225-X15: unrestricted exact separators for actual validated H225-X14 survivors."""
...
X14=ROOT/'data'/'derived'/'h225_x14_incremental_exact_rescreen.json'
OUTDIR=ROOT/'data'/'derived'/'h225_x15_separator_shards'
MERGED=ROOT/'data'/'derived'/'h225_x15_survivor_separation.json'
NEW=ROOT/'data'/'derived'/'h225_x15_new_witnesses.json'
base.X8=X14
base.OUTDIR=OUTDIR
base.MERGED=MERGED
base.NEW=NEW
...
def load_x14():
 d=json.loads(X14.read_text())
 assert d['packet']=='H225-X14' and d['chunk_shards']==44 and d['quotient_coefficient_states_screened']==306450 and len(d['sectors'])==11
 ...
 assert int(d['exact_shift_surviving_coefficient_states'])>0
 return d
base.load_x8=load_x14

def relabel_job(out):
 out['packet']='H225-X15'; out['source_packet']='H225-X14'
 if 'x8_chunk_survivor_states' in out: out['x14_chunk_survivor_states']=out.pop('x8_chunk_survivor_states')
 if 'x8_chunk_survivor_shift_tuples' in out: out['x14_chunk_survivor_shift_tuples']=out.pop('x8_chunk_survivor_shift_tuples')
 return out

def solve_job(job,time_limit): return relabel_job(base.solve_job(job,time_limit))

def merge(paths):
 merged,wp=base.merge(paths); x14=load_x14()
 merged['packet']='H225-X15'; merged['source_packet']='H225-X14'
 merged['x14_survivor_states']=int(x14['exact_shift_surviving_coefficient_states'])
 merged['x14_survivor_shift_tuples']=int(x14['exact_surviving_shift_tuples'])
 merged.pop('x8_survivor_states',None); merged.pop('x8_survivor_shift_tuples',None)
 for r in merged['jobs']: relabel_job(r)
 wp['packet']='H225-X15'; wp['deduplicated_against']=['H234','H225-X1','H225-X3','H225-X5','H225-X7','H225-X9','H225-X11','H225-X13']
 return merged,wp
