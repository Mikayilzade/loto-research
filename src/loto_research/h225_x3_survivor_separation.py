...
The H225-X2 result is currently stored under the legacy filename
h241_h240_incremental_exact_rescreen.json. One actual surviving shift-level design
is selected from each chunk that still has survivors. Chunks with zero survivors
...
def load_x2():
    ...
    assert d['packet']=='H241' and d['chunk_shards']==44 and d['quotient_coefficient_states_screened']==306450
    return d

def solve_job(job,time_limit):
    assert 0<=job<JOBS
    si,ci=divmod(job,4); d=load_x2(); sec=d['sectors'][si]; ch=sec['chunks'][ci]
    assert (ch['sector_index'],ch['chunk_index'])==(si,ci)
    if ch['exact_shift_surviving_coefficient_states']==0:
        return {'packet':'H225-X3','job':job,'sector_index':si,'chunk_index':ci,'skipped_no_survivor':True,
                'counterexample_found':False,'conclusive_for_selected_design':True,'n3':None,'witness':None}
    survivors=ch['first_exact_survivors']; assert survivors
    chosen=survivors[0]; coefficients=tuple(map(int,chosen['coefficients'])); shifts=tuple(map(int,chosen['first_shift_examples'][0]))
    beta,gamma=map(int,sec['representative']); T=build_triples(beta,gamma,coefficients,shifts)
...
def merge(paths):
    ...
    merged={'packet':'H225-X3','source_packet':'H225-X2','source_legacy_file':str(X2.relative_to(ROOT)),
