"""H225-X8: full exact rescreen after H225-X7 separator cuts."""
...
OUTDIR=ROOT/'data'/'derived'/'h225_x8_chunk_shards'
MERGED=ROOT/'data'/'derived'/'h225_x8_incremental_exact_rescreen.json'
...
def solve_chunk(sector_index,chunk):
 ...
 return {'packet':'H225-X8','source_packet':'H225-X7','sector_index':sector_index,'chunk_index':chunk,'representative':[int(beta),int(gamma)],'sector_total_quotient_states':len(reps),'chunk_start':lo,'chunk_stop':hi,'chunk_quotient_states':len(local),'ordered_sector_orbit_size':len(orb),'ordered_sector_stabilizer_size':stab,'distinct_A_coefficient_actions':actions,'base_expanded_witness_instances':int(len(W)-sum(exp)),'raw_witness_counts':raw,'affine_expanded_counts':exp,'x7_raw_new_witnesses':raw[-1],'x7_affine_expanded_instances':exp[-1],'augmented_witness_instances_before_signature_dedupe':int(len(W)),'general_signature_unique_witnesses':int(len(A)),'h185_stored':int(pre_n),'h186_witnesses':int(h186_n),'envelope_survivor_states':int(env),'exact_shift_surviving_coefficient_states':int(states),'exact_surviving_shift_tuples':int(tuples),'envelope_first_killer_histogram':{str(k):int(v) for k,v in sorted(ek.items())},'csp_terminal_killer_histogram':{str(k):int(v) for k,v in sorted(ck.items())},'first_exact_survivors':examples}

def merge(paths):
 rows=sorted([json.loads(p.read_text()) for p in paths],key=lambda x:(x['sector_index'],x['chunk_index'])); assert len(rows)==44 and all(x['packet']=='H225-X8' for x in rows); assert [(x['sector_index'],x['chunk_index']) for x in rows]==[(s,c) for s in range(11) for c in range(CHUNKS)]
 sectors=[]
 for s in range(11):
 ...
 return {'packet':'H225-X8','method':'full_exact_rescreen_after_X7_new_witness_affine_orbits','source_packet':'H225-X7','chunk_shards':44,'quotient_coefficient_states_screened':total,'envelope_survivor_states':sum(x['envelope_survivor_states'] for x in sectors),'exact_shift_surviving_coefficient_states':exstates,'exact_surviving_shift_tuples':sum(x['exact_surviving_shift_tuples'] for x in sectors),'x7_raw_new_witnesses':rows[0]['x7_raw_new_witnesses'],'x7_affine_expanded_instances':rows[0]['x7_affine_expanded_instances'],'all_general_cyclic_affine_designs_rejected_by_augmented_witnesses':bool(exstates==0),'sectors':sectors,'interpretation':'Zero exact survivors closes H225. Positive survivors require H225-X9 separators.'}
