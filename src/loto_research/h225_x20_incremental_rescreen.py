...
import loto_research.h225_x18_incremental_rescreen as base
...
base.PACKETS=base.PACKETS+[(X19,'H225-X19')]

def solve_chunk(sector_index,chunk):
 out=base.solve_chunk(sector_index,chunk)
 out['packet']='H225-X20'; out['source_packet']='H225-X19'
 out['x19_raw_new_witnesses']=out.pop('x17_raw_new_witnesses')
 out['x19_affine_expanded_instances']=out.pop('x17_affine_expanded_instances')
 return out
...
def merge(paths):
 ...
 assert [(x['sector_index'],x['chunk_index']) for x in rows]==[(s,c) for s in range(11) for c in range(base.base.CHUNKS)]
 sectors=[]
 for s in range(11):
  rr=[x for x in rows if x['sector_index']==s]
  assert sum(int(x['chunk_quotient_states']) for x in rr)==base.base.EXPECTED_STATES[s]
  sectors.append({'sector_index':s,'representative':list(base.base.EXPECTED_REPS[s]),'quotient_coefficient_states':base.base.EXPECTED_STATES[s],
...
def main():
 ...
 if a.sector is not None:
  assert a.chunk is not None and 0<=a.sector<11 and 0<=a.chunk<base.base.CHUNKS
